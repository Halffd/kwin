    ...
    , m_flags(flags)
{
    ...
    if (!m_fd.isValid()) {
        qWarning().nospace() << name << ": Can't create memfd: " << strerror(errno);
        return;
    }

    if (ftruncate(m_fd.get(), size) < 0) {
        qWarning().nospace() << name << ": Failed to ftruncate memfd: " << strerror(errno);
        return;
    }

    void *data = mmap(nullptr, size, PROT_READ | PROT_WRITE, MAP_SHARED, m_fd.get(), 0);
    if (data == MAP_FAILED) {
        qWarning().nospace() << name << ": mmap failed: " << strerror(errno);
        return;
    }
#else
    m_tmp = std::make_unique<QTemporaryFile>();
    if (!m_tmp->open()) {
        qWarning().nospace() << name << ": Can't open temporary file";
        return;
    }

    if (unlink(m_tmp->fileName().toUtf8().constData()) != 0) {
        qWarning().nospace() << name << ": Failed to remove temporary file from filesystem: " << strerror(errno);
    }

    if (!m_tmp->resize(size)) {
        qWarning().nospace() << name << ": Failed to resize temporary file";
        return;
    }

    uchar *data = m_tmp->map(0, size);
    if (!data) {
        qWarning().nospace() << name << ": map failed";
        return;
    }
#endif
    ...
    if (fcntl(fd(), F_ADD_SEALS, seals) != 0) {
        qDebug().nospace() << name << ": Failed to seal RamFile: " << strerror(errno);
    }
