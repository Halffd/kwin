                                                                        ...
                                                                        const QSize &size, VkFormat format)
{
    ...
    if (!fb->m_colorTexture) {
        qWarning() << "Failed to create color texture for framebuffer";
        return nullptr;
    }
    ...
    if (renderPass->config().hasDepth) {
        ...
        if (!fb->m_depthTexture) {
            qWarning() << "Failed to create depth texture for framebuffer";
            return nullptr;
        }
...
bool VulkanFramebuffer::createFramebuffer(const std::vector<VkImageView> &attachments)
{
    ...
    if (result != VK_SUCCESS) {
        qWarning() << "Failed to create framebuffer:" << result;
        return false;
    }

    qDebug() << "Successfully created framebuffer with" << attachments.size() << "attachments";
    return true;
}
                                 ...
                                 const QRect &destRect, VkFilter filter)
{
    if (!source || !source->m_colorTexture || !m_colorTexture) {
        qWarning() << "Cannot blit: source or destination texture missing";
        return;
    }
