...
bool VulkanAllocator::initialize(VulkanBackend *backend)
{
    ...
    if (result != VK_SUCCESS) {
        qWarning() << "Failed to create VMA allocator:" << result;
        return false;
    }

    s_initialized = true;
    qDebug() << "VMA allocator initialized successfully";
    return true;
}
                                                   ...
                                                   UsageHint usageHint, bool persistentMap)
{
    if (!VulkanAllocator::isInitialized()) {
        qWarning() << "VMA allocator not initialized";
        return nullptr;
    }
    ...
    if (result != VK_SUCCESS) {
        qWarning() << "Failed to create Vulkan buffer:" << result;
        return nullptr;
    }
...
void VulkanBuffer::upload(const void *data, VkDeviceSize dataSize, VkDeviceSize offset)
{
    if (dataSize + offset > m_size) {
        qWarning() << "Buffer upload exceeds buffer size";
        return;
    }
    ...
    if (m_persistentlyMapped && m_mappedData) {
        ...
        if (!staging) {
            qWarning() << "Failed to create staging buffer for upload";
            return;
        }
...
void *VulkanBuffer::map()
{
    ...
    if (result != VK_SUCCESS) {
        qWarning() << "Failed to map buffer:" << result;
        return nullptr;
    }
...
void *VulkanBuffer::allocate(VkDeviceSize size, VkDeviceSize alignment)
{
    ...
    if (alignedOffset + size > m_size) {
        qWarning() << "Streaming buffer exhausted";
        return nullptr;
    }
