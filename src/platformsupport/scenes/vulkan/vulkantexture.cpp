...
bool VulkanTexture::createImage(const QSize &size, VkFormat format, VkImageUsageFlags usage, VkImageTiling tiling)
{
    ...
    if (result != VK_SUCCESS) {
        qWarning() << "Failed to create Vulkan image:" << result;
        return false;
    }
...
bool VulkanTexture::createImageView(VkImageAspectFlags aspectFlags)
{
    ...
    if (result != VK_SUCCESS) {
        qWarning() << "Failed to create Vulkan image view:" << result;
        return false;
    }
...
bool VulkanTexture::createSampler()
{
    ...
    if (result != VK_SUCCESS) {
        qWarning() << "Failed to create Vulkan sampler:" << result;
        return false;
    }
...
std::unique_ptr<VulkanTexture> VulkanTexture::upload(VulkanContext *context, const QImage &image)
{
    ...
    if (!texture->createImage(image.size(), format,
        ...
        qWarning() << "Failed to create Vulkan texture image for upload";
        return nullptr;
    }

    if (!texture->createImageView()) {
        qWarning() << "Failed to create Vulkan texture image view for upload";
        return nullptr;
    }

    if (!texture->createSampler()) {
        qWarning() << "Failed to create Vulkan texture sampler for upload";
        return nullptr;
    }
...
std::unique_ptr<VulkanTexture> VulkanTexture::createDepthStencil(VulkanContext *context, const QSize &size)
{
    ...
    if (depthFormat == VK_FORMAT_UNDEFINED) {
        qWarning() << "No suitable depth format found";
        return nullptr;
    }
...
void VulkanTexture::update(const QImage &image, const QRegion &region)
{
    ...
    if (!staging) {
        qWarning() << "Failed to create staging buffer for texture update";
        return;
    }

    void *mapped = staging->map();
    if (!mapped) {
        qWarning() << "Failed to map staging buffer for texture update";
        return;
    }
    ...
    if (cmd == VK_NULL_HANDLE) {
        qWarning() << "Failed to begin single time commands for texture update";
        return;
    }
