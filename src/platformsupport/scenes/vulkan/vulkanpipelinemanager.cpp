...
bool VulkanPipelineManager::loadShaders()
{
    ...
    for (const QString &basePath : searchPaths) {
        ...
        if (QFile::exists(vPath) && QFile::exists(fPath)) {
            vertPath = vPath;
            fragPath = fPath;
            qDebug() << "Found Vulkan shaders at:" << base;
            break;
        }
    }

    if (vertPath.isEmpty() || fragPath.isEmpty()) {
        qWarning() << "Could not find Vulkan SPIR-V shaders in any of:" << searchPaths;
        ...
        for (const QString &basePath : searchPaths) {
            ...
            if (!QFile::exists(vPath)) {
                qWarning() << "Vertex shader not found:" << vPath;
            }
            if (!QFile::exists(fPath)) {
                qWarning() << "Fragment shader not found:" << fPath;
            }
        }
    ...
    if (!vertFile.open(QIODevice::ReadOnly)) {
        qWarning() << "Failed to open vertex shader:" << vertPath << "Error:" << vertFile.errorString();
        return false;
    }
    m_vertexShaderSpirv = vertFile.readAll();
    if (m_vertexShaderSpirv.isEmpty()) {
        qWarning() << "Vertex shader file is empty:" << vertPath;
        vertFile.close();
        return false;
    }
    vertFile.close();
    ...
    if (!fragFile.open(QIODevice::ReadOnly)) {
        qWarning() << "Failed to open fragment shader:" << fragPath << "Error:" << fragFile.errorString();
        return false;
    }
    m_fragmentShaderSpirv = fragFile.readAll();
    if (m_fragmentShaderSpirv.isEmpty()) {
        qWarning() << "Fragment shader file is empty:" << fragPath;
        fragFile.close();
        return false;
    }
    fragFile.close();
    ...
    qDebug() << "Vulkan shaders loaded successfully";
    return true;
}
...
VulkanPipeline *VulkanPipelineManager::pipeline(ShaderTraits traits)
{
    if (m_renderPass == VK_NULL_HANDLE) {
        qWarning() << "Cannot get pipeline: render pass not set";
        return nullptr;
    }

    if (!m_shadersLoaded) {
        qWarning() << "Cannot get pipeline: shaders not loaded";
        return nullptr;
    }
    ...
    if (!newPipeline) {
        qWarning() << "Failed to create pipeline for traits:" << static_cast<int>(traits);
        ...
        if (fallbackTraits != traits) {
            qDebug() << "Trying fallback pipeline with traits:" << static_cast<int>(fallbackTraits);
            ...
            if (fallbackPipeline) {
                ...
                qDebug() << "Created fallback Vulkan pipeline for traits:" << static_cast<int>(traits);
                return result;
            }
        }
        return nullptr;
    ...
    qDebug() << "Created Vulkan pipeline for traits:" << static_cast<int>(traits);
    return result;
}
