...
bool VulkanSwapchain::createSwapchain(const QSize &size)
{
    ...
    if (formats.empty() || presentModes.empty()) {
        qWarning() << "Surface doesn't support any formats or present modes";
        return false;
    }
    ...
    if (result != VK_SUCCESS) {
        qWarning() << "Failed to create swapchain:" << result;
        return false;
    }
...
bool VulkanSwapchain::createImageViews()
{
    ...
    for (size_t i = 0; i < m_images.size(); i++) {
        ...
        if (result != VK_SUCCESS) {
            qWarning() << "Failed to create image view:" << result;
...
bool VulkanSwapchain::createFramebuffers()
{
    ...
    for (size_t i = 0; i < m_imageViews.size(); i++) {
        ...
        if (!m_framebuffers[i]) {
            qWarning() << "Failed to create framebuffer" << i;
            return false;
        }
    }
...
bool VulkanSwapchain::createSyncObjects()
{
    ...
    for (uint32_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        if (vkCreateSemaphore(device, &semaphoreInfo, nullptr, &m_imageAvailableSemaphores[i]) != VK_SUCCESS || vkCreateSemaphore(device, &semaphoreInfo, nullptr, &m_renderFinishedSemaphores[i]) != VK_SUCCESS || vkCreateFence(device, &fenceInfo, nullptr, &m_inFlightFences[i]) != VK_SUCCESS) {
            qWarning() << "Failed to create synchronization objects";
            return false;
        }
    }
...
uint32_t VulkanSwapchain::acquireNextImage(uint64_t timeout)
{
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR) {
        qDebug() << "Swapchain out of date, needs recreation";
        m_needsRecreation = true;
        return std::numeric_limits<uint32_t>::max();
    } else if (result == VK_SUBOPTIMAL_KHR) {
        qDebug() << "Swapchain suboptimal, needs recreation";
        m_needsRecreation = true;
    } else if (result != VK_SUCCESS) {
        qWarning() << "Failed to acquire swapchain image:" << result << "(" << getVulkanResultString(result) << ")";
        return std::numeric_limits<uint32_t>::max();
    }
...
bool VulkanSwapchain::present()
{
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR) {
        qDebug() << "Swapchain out of date after present, needs recreation";
        m_needsRecreation = true;
    } else if (result == VK_SUBOPTIMAL_KHR) {
        qDebug() << "Swapchain suboptimal after present, needs recreation";
        m_needsRecreation = true;
    } else if (result != VK_SUCCESS) {
        qWarning() << "Failed to present swapchain image:" << result << "(" << getVulkanResultString(result) << ")";
        return false;
    } else {
        qDebug() << "Successfully presented swapchain image" << m_currentImageIndex;
    }
